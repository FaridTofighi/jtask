...
| column sort | مرتب‌سازی کارت‌ها | Sort cards | per-column card order, chosen from a header menu: «فوریت: زیاد ← کم» / «فوریت: کم ← زیاد» / «جدیدترین اول» / «قدیمی‌ترین اول» (`board.sort.*`). Default `urgency-`. Reuses the task's own `urgency` + `entry`. |
