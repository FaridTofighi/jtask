...
| `taskwarrior.py` | the **only** subprocess wrapper for `task` | `run(args, *, quiet=…)` (raises `TaskCommandError` on non-zero), `export(filter, *, apply_context=True)`, `add`, `command(filter, verb, mods)`, `_lines`, `sync_status`, `synchronize`, `purge`, `duplicate`, `information`, `diagnostics`, `calc`. `export`/`export_text` prepend the active context's read filter (`context_read_filter()`) because `task export` ignores context; `apply_context=False` = full data (backups). Always passes `_RC` = `rc.confirmation=off rc.recurrence.confirmation=off rc.bulk=0 rc.color=off rc.hooks=on rc.search.case.sensitive=no`; adds `rc.verbose=nothing` when `quiet`. |
